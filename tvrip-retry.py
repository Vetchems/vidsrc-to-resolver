...
if __name__ == "__main__":
    ...
    def download_episode(season, episode_number):
        ...
        season = int(season)
        filename = f"{cleaned_text}.S{season:02d}E{episode_number:02d}"

        tv_dir = "TV"
        os.makedirs(tv_dir, exist_ok=True)
        series_dir = os.path.join(tv_dir, cleaned_text)
        os.makedirs(series_dir, exist_ok=True)
        season_dir = os.path.join(series_dir, f"S{season:02d}")
    ...
    if media_type == "tv":
        for episode_number in range(int(ep), int(end_ep) + 1):
            success = download_episode(se, episode_number)
            if not success:
        ...
        if failed_downloads:
            ...
            for season, episode_number in failed_downloads:
                success = download_episode(season, episode_number)
                if success:
                    failed_downloads.remove((season, episode_number))

        if failed_downloads:
            print("\nThe following episodes failed to download:")
            for season, episode_number in failed_downloads:
                print(f"Season {season}, Episode {episode_number}")
        else:
